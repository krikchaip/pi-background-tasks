...
| Fusion child isolation/lifecycle | child `pi --mode json`, shutdown cleanup, `.pi/fusion` artifacts | yes | yes | yes |  |  | yes |  | Unit covers argv, env cleanup, stdin transport, LF JSONL parser, stdin failure, stdout/stderr output caps, timeout/abort/process-group kill, abort-during-spawn race, cleanup failure surfacing, no-close-after-SIGKILL failure, manifest transitions, atomic artifact writes, failed-run usage persistence, aggregate manifest usage exactly matching successful plus observed failed/cancelled attempt usage for candidate/evaluator/repair/merger failures, failed-attempt response artifact references, transient spawn retry, candidate sibling cancellation, concurrent workflows, and orchestrator cancellation. SDK/RPC verify actual public command/tool consumers launch direct child `pi` with all isolation flags. Session shutdown tracks initializing and live Fusion runs through the extension active-run set. |
